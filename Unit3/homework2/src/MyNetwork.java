...
public class MyNetwork implements Network {
    ...
    private int coupleSum;      // number of couple relations, dynamically maintained
    private final HashMap<Integer, MyPerson> persons = new HashMap<>();
    private int lastBlockSum;   // number of connected blocks, if -1, need to recalculate
    ...
    private void update(MyPerson p1, MyPerson p2, MyPerson oldCp1, MyPerson oldCp2, int op) {
        if (op == ADD || op == DELETE) {
            int tripleSumDelta = (int) persons.values().stream()
                    ...
                    .count();
            tripleSum = tripleSum + (op == ADD ? tripleSumDelta : -tripleSumDelta);
            lastBlockSum = -1;
        }
