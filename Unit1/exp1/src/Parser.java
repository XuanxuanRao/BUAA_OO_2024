...
public class Parser {
    //private  final Pattern numberPattern = Pattern.compile("[\\da-zA-Z]+"); // TODO
        ...
        if (position != -1) {
            ...
            if (position != -1) {
                ...
                if (!expression.equals("")) {
                    Pattern numberPattern = Pattern.compile("[\\da-zA-Z]+");
                    Matcher matcher = numberPattern.matcher(expression);
                    if (matcher.find()) {
                        String numberStr = matcher.group(0);
                        if (Character.isDigit(numberStr.charAt(0))) {
                            return new Num(Integer.parseInt(numberStr));
                        } else {
                            return new Num(parameters.get(numberStr));
                        }
                    } else {
                        return new Num(0);
                    }
